...
<link rel='import'
	href='our_components/gm-circle-progress/gm-circle-progress.html' />
<link rel='import'
	href='our_components/gm-navigation-tab/gm-navigation-tab.html' />
<link rel='import'
...
<body>
	...
	<div id="after-loading" style="height: 100%; margin: 0; display: none">
		...
		<myunimol-ajax auto id='ajax' method='POST' url="HomeServlet"
 ...
 %>
		...
		<core-header-panel main class="main-panel"> <core-toolbar
			id="mainheader"> <paper-icon-button id="navicon"
			icon="menu"></paper-icon-button> <span flex style="font-size: 28;">
			Home </span> </core-toolbar>
		<div id='activeContentHandler' class="content">
			<div id="studentInfo">
				<h1>
					<%=utils.getUser().getName() + " "
					+ utils.getUser().getSurname()%>
				</h1>
				<h3>
					<%=utils.getUser().getStudentId()%>
				</h3>
			</div>
			<div id="studentProgress">
				...
				<div id="student-media-ponderata">
					...
					<p id="weightedAverage"></p>
				</div>
				<hr>
				<div>
					<h3>Esami Registrati</h3>
					<p id="acquiredExams"></p>
				</div>
				<hr>
				</core-collapse>
						...
						function() {
							...
							ajax
									.addEventListener(
											"myunimol-response",
											function(event) {
												var circleProgressBar = document
														.getElementsByTagName("gm-circle-progress")[0];
												circleProgressBar
														.setAttribute(
																"percentage",
																event.detail.response.percentCFU);
												circleProgressBar
														.setAttribute(
																"numcfu",
																event.detail.response.acquiredCFU
																		+ "/"
																		+ event.detail.response.totalCFU);
												document
														.getElementById("average").innerHTML = event.detail.response.average;
												document
														.getElementById("weightedAverage").innerHTML = event.detail.response.weightedAverage;
												document
														.getElementById("acquiredExams").innerHTML = ""
														+ event.detail.response.totalExams;
