...
<body id="body_id">
	...
	<div id="after-loading" style="height: 100%; margin: 0; display: none">
		...
		<div id="push1"></div>
		<div id="content">
			<paper-input-decorator type="text" label="Username"
				id="username_input" floatinglabel class="my-input"
				error="Inserisci il nome utente!"> <input
				is="core-input" tabindex="1" id="username_input_field"
				autocapitalize="off" required> </paper-input-decorator>
			<paper-input-decorator type="password" id="password_input"
				label="Password" floatinglabel class="my-input"
				error="Inserisci la password!"> <input
				is="core-input" tabindex="2" id="password_input_field"
				type="password" autocapitalize="off" required> </paper-input-decorator>
			<div id="buttons">
		...
		<paper-dialog id="privacyDialog" heading="Privacy"
			transition="core-transition-center">
		<h2>Condizioni di servizio</h2>
		<div style="text-align: justify;">
			<p>ATTENZIONE: MyUnimol non &egrave; un'applicazione ufficiale
				dell'Universit&agrave; degli Studi del Molise. Tutti i dati vengono
				reperiti dalla piattaforma ESSE3 dell'Universit&agrave; degli Studi
				del Molise e tali informazioni non vengono memorizzate nei nostri
				sistemi.</p>
			<p>Effettuando l'accesso si esprime il proprio consenso al
				download automatico dei propri dati (quali nome, cognome, voto degli
				esami, corso di studi...) dalla piattaforma ESSE3. MyUnimol si
				solleva da ogni eventuale responsabilit&agrave;.</p>
		</div>
		</paper-dialog>
		<myunimol-ajax nofreeze id='ajaxData' method='POST' url='LoginServlet'
			params='{"username":"", "password":""}' handleAs='json'
	...
	<script>
		function sendData() {
			...
			if (username != '' && password != '') {
				...
				reqst.params = '{"username": "' + username.toLowerCase()
						+ '", "password":"' + password + '"}';
				reqst.go();
				freeze("content");
			}
			var $d = document.getElementById('body_id').querySelectorAll(
		...
		document
				.addEventListener(
						...
						function() {
							...
							var mobileFooterDiv = document
									.getElementById("mobile_footer");
									...
									.addEventListener(
											...
											function() {
												...
												if (mobileDisclamer.style.display === 'none') {
													...
													document
															.getElementById('buttons').style.display = "block";
							...
							document.getElementById("username_input_field")
									.addEventListener("keypress", keyPressed)
							document.getElementById("password_input_field")
									.addEventListener("keypress", keyPressed)
						});
