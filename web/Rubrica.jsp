...
<body>
	...
	<div id="after-loading" style="height: 100%; margin: 0; display: none">
		...
		<myunimol-ajax auto id='ajax' method='POST'
			url="GetAddressBookServlet" params='{}' handleAs='json'>
		</myunimol-ajax>

		<myunimol-ajax id='ajaxSearch' method='POST'
			url="SearchContactsServlet" params='{}' handleAs='json'>
		</myunimol-ajax>
 ...
 %>
		...
		<core-header-panel main> <core-toolbar
			...
			icon="menu"></paper-icon-button> <paper-icon-button id="backButton"
			icon="arrow-back" style='display:none; '></paper-icon-button> <span
			id='mainToolbar' flex style="font-size: 28;"> <span
			id='labelRubrica'> Rubrica </span> <input
			style='color: white; display: none;' type='text' label=''
			id='ricercaInput' class='my-input'>
		</paper-input>
		</span> <paper-icon-button id='buttonRicercaShow' style="color: white;"
			id="paper_item" icon="search"></paper-icon-button> <paper-icon-button
			id='buttonRicercaDo' style="color: white; display:none;"
			id="paper_item" icon="search"></paper-icon-button> </core-toolbar>
		...
		<div id='activeContentHandler' class="content">
			...
			<div class="centeredMessage" id="no-contacts" style="display: none;">
				...
				<p>
					Spiacente ma non riesco a trovare contatti cercando "<span></span>"...
				</p>
			</div>
		</div>
		...
		document
				.addEventListener(
						...
						function() {
							...
							document.efcontact = document
									.getElementById("efcontact");
							document.noContacts = document
									.getElementById("no-contacts");
							...
							document.ricercaShow = document
									.getElementById("buttonRicercaShow");
							document.ricercaDo = document
									.getElementById("buttonRicercaDo");
							document.navicon = document
									.getElementById('navicon');
							document.backButton = document
									.getElementById('backButton');
							document.labelRubrica = document
									.getElementById("labelRubrica");
							document.ricercaInput = document
									.getElementById('ricercaInput');
							...
							document.ricercaDoAction = function() {
								var ajax = document
										.getElementById('ajaxSearch');
								ajax.params = '{"search":"'
										+ document.ricercaInput.value + '"}'
								ajax.addEventListener('myunimol-response',
										document.ricercaResults);
								ajax.go();
							...
							document.ricercaShow.addEventListener('click',
									document.ricercaShowAction);
							document.backButton.addEventListener('click',
									document.backAction);
							document.ricercaDo.addEventListener('click',
									document.ricercaDoAction);
							document.ricercaShow.addEventListener('click',
									document.ricercaShowAction);

							document.ricercaInput.addEventListener('keypress',
									function(e) {
